...
function CreateFrame(frameType, name, parent, template)
    ...
    function frame:EnableMouse(b)
        self.mouseEnabled = b
    end
    function frame:SetMovable(b)
        self.movable = b
    end
    function frame:RegisterForDrag(...)
        self.dragEvents = { ... }
    end
    function frame:SetBackdrop(bdrop)
        self.backdrop = bdrop
    end
    ...
    function frame:SetPropagateKeyboardInput(b)
        self.propagateKeyboard = b
    end
    ...
    function frame:HookScript(event, func)
        self.scripts[event] = func
    end

    -- Event registration.
    frame.registeredEvents = {}
    function frame:RegisterEvent(evt)
        self.registeredEvents[evt] = true
    end
    function frame:UnregisterEvent(evt)
    ...
    if template and template:find("BasicFrameTemplate") then
        frame.TitleText = { text = "", SetText = function(self, txt)
            self.text = txt
    ...
    if template and template:find("InterfaceOptionsCheckButtonTemplate") then
        ...
        frame.Text = { text = "", SetText = function(self, txt)
            self.text = txt
        end }
    end
    ...
    function frame:CreateTexture(n, layer, templ)
        ...
        function tex:SetRotation(angle)
            self.rotation = angle
        end
